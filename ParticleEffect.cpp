...
void ParticleEffect::RandomizeParticle( Particle& particle)
{
	...
	//particle.m_Velocity = localVelovity;
}
...
void ParticleEffect::Update(float fDeltaTime)
{
	for(unsigned int i = 0; i < m_Particles.size(); ++i)
	{
		...
		float lifeRatio = glm::saturate<GLfloat, glm::highp>(particle.m_fAge / particle.m_fLifeTime);
...
// velocity u/v/t
void ParticleEffect::Update(float fDeltaTime, const glm::vec3& localVelovity,
							const glm::vec3& uLocal,
							const glm::vec3& vLocal,
							const glm::vec3& tLocal)
{
    for ( unsigned int i = 0; i < m_Particles.size(); ++i )
    {
        Particle& particle = m_Particles[i];
        ...
        //particle.m_Position += ( particle.m_Velocity * fDeltaTime );
		particle.m_Position += (uLocal * particle.m_Velocity.x * fDeltaTime +
								vLocal * particle.m_Velocity.y * fDeltaTime +
								tLocal * particle.m_Velocity.z * fDeltaTime);
...
void ParticleEffect::Render()
{
	...
	glBegin(GL_QUADS);
	for(const auto& i : m_VertexBuffer) {
		glPushAttrib(GL_LIGHTING);
		GLfloat matDiffuse[4] = { i.m_Diffuse.r, i.m_Diffuse.g, i.m_Diffuse.b, i.m_Diffuse.a };
		...
		glVertex3f(i.m_Pos.x, i.m_Pos.y, i.m_Pos.z);
		glPopAttrib();
	}
	glEnd();
